...
function createEnemyAt(x,y,zone=null,kind=null){
  ...
  enemies.push({
    ...
    bloodColor: (spriteIndex===2) ? 'yellow' : 'red',
...
function buildMap(){
  ...
  // tlačítka jsou na zdi u hlavních vchodů, ne uprostřed podlahy
  alarmButton(125,650,0);
  alarmButton(790,410,Math.PI);
  alarmButton(1450,375,Math.PI);
  alarmButton(2130,720,-Math.PI/2);
  alarmButton(1020,1020,-Math.PI/2);
  alarmButton(2070,1020,-Math.PI/2);
  alarmButton(1070,1660,-Math.PI/2);
  alarmButton(2050,1660,-Math.PI/2);
...
function initLobbyTargets(){
  ...
  lobbyTargets.push({name:'FIGURÍNA',x:lobbyData.targetA.x,y:lobbyData.targetA.y,ang:lobbyData.targetA.ang,r:20,hp:80,maxHp:80,spriteIndex:0,state:'idle',bloodColor:'red',blood:[],respawn:0,muzzleFx:0});
  lobbyTargets.push({name:'ČERNÝ TERČ',x:lobbyData.targetB.x,y:lobbyData.targetB.y,ang:lobbyData.targetB.ang,r:20,hp:105,maxHp:105,spriteIndex:2,state:'idle',bloodColor:'yellow',blood:[],respawn:0,muzzleFx:0});
}
...
function hitLobbyTarget(t,b){
  ...
  const col=t.bloodColor || (t.spriteIndex===2?'yellow':'red');
  t.hp -= b.dmg;
...
function drawLobbyWindows(){
  ...
  if(lobbyUi==='shop'){
    ...
    if(loadoutCategory==='weapons'){
      ...
      if(!cards.length){
        ctx.font='26px Impact'; ctx.fillStyle='rgba(255,255,255,.55)';
        ctx.fillText('PRÁZDNÉ',W/2,350);
      }
...
function sanitizeInventorySave(){
  const valid = ['m4','sniper','lmg','shotgun'];
  if(!Array.isArray(player.loadoutSlots)) player.loadoutSlots = ['m4', null];
  player.loadoutSlots = [player.loadoutSlots[0], player.loadoutSlots[1]].map(k => valid.includes(k) ? k : null);
  if(!player.loadoutSlots[0] && !player.loadoutSlots[1]) player.loadoutSlots[0] = 'm4';
  player.selectedSlot = clamp(player.selectedSlot|0,0,1);
  if(!player.loadoutSlots[player.selectedSlot]) player.selectedSlot = player.loadoutSlots[0] ? 0 : 1;
  if(!valid.includes(player.weaponType)) player.weaponType = player.loadoutSlots[player.selectedSlot] || player.loadoutSlots[0] || 'm4';
  player.slotTroll = [!!(player.slotTroll&&player.slotTroll[0]), !!(player.slotTroll&&player.slotTroll[1])];
  player.equippedGear = {};
  for(const k of Object.keys(player.weaponState||{})) if(!valid.includes(k)) delete player.weaponState[k];
}
function loadProgress(){
  try{
    const raw = localStorage.getItem(SAVE_KEY);
    if(!raw){ sanitizeInventorySave(); return; }
    ...
    if(Array.isArray(data.loadoutSlots)) player.loadoutSlots = [data.loadoutSlots[0] || 'm4', data.loadoutSlots[1] || null];
...
function updatePlayerSprite(){
  ...
  const cached = getSpriteAssetFor(player.characterKey, player.weaponType);
  if(cached && cached.ready && cached.img){
    playerSpriteReady = true;
    playerSprite.src = cached.img.src;
  } else if(playerSprite.src !== spriteSrc){
    playerSpriteReady = false;
    playerSprite.src = spriteSrc;
  }
  enemySpritesReady[3] = !!(cached && cached.ready && cached.img);
  enemySprites[3].src = (cached && cached.img && cached.img.src) ? cached.img.src : spriteSrc;
}
...
function applyLoadout(key, save=true, refill=false, troll=false){
  const useKey = (weaponDefs[key] && key !== 'grenade') ? key : 'm4';
  snapshotWeaponState();
  player.selectedSlot = clamp(player.selectedSlot|0,0,1);
...
function switchInventorySlot(slot){
  ...
  if(!key || !weaponDefs[key] || key==='grenade'){ message='PRÁZDNÝ SLOT'; messageTimer=.8; return false; }
  snapshotWeaponState();
...
function setInventoryWeaponFromLoadout(key){
  if(!weaponDefs[key] || key==='grenade') return;
  snapshotWeaponState();
...
function startCameraShake(power=8,dur=.22){
  if(power >= 12) dur = Math.min(dur, 2.0);
...
function addBloodDecal(x,y,size=10,onWall=false,a=0,color='red'){
  ...
  if(bloodDecals.length > 210) bloodDecals.splice(0, bloodDecals.length - 210);
}
...
function drawPlayer(){
  ...
  const asset = getSpriteAssetFor(player.characterKey, player.weaponType);
  const pImg = (asset && asset.ready && asset.img) ? asset.img : (playerSpriteReady ? playerSprite : null);
  if(pImg){
    ctx.imageSmoothingEnabled = true;
    try { ctx.imageSmoothingQuality = 'high'; } catch(e) {}
    ctx.filter = player.hiddenInShadow ? 'brightness(0.10) contrast(1.35) saturate(.25)' : 'brightness(0.40) contrast(1.18) saturate(.78)';
    ctx.drawImage(pImg, -74, -74 + bob, 148, 148);
    ctx.filter='none';
  } else {
    ctx.fillStyle='#d9d9d9'; ctx.strokeStyle='#000'; ctx.lineWidth=4;
...
function drawInventoryBar(){
  if(state!=='play') return;
  const size=96, gap=22, x=W/2 - size - gap/2, y=H-124;
  ctx.save();
  ctx.textAlign='center';
  for(let i=0;i<2;i++){
    ...
    if(key && loadoutDefs[key]){
      const d=loadoutDefs[key];
      if(d.ready && d.imgPath) ctx.drawImage(d.img,sx+10,y+12,size-20,50); else drawItemIcon(key,sx+10,y+10,size-20,52);
      ctx.font='15px Impact'; ctx.fillStyle='#fff';
      ctx.fillText(d.name,sx+size/2,y+72);
