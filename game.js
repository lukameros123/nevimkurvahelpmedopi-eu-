...
async function trackOnlineRoom(){
  ...
  await online.indexChannel.track({
    type:'room',
    roomId:online.roomId,
    code:online.roomCode,
    host:online.playerName,
    players,
...
function onlineTick(dt){
  if(online.isHost && online.indexChannel && online.roomId){
    if(performance.now() - online.lastIndexTrack > 1800){
...
const enemySprites = [new Image(), new Image(), new Image(), new Image()];
const enemySpritesReady = [false, false, false, false];
...
const loadoutOrder = ['m4','sniper','lmg','shotgun'];
...
function alarmButton(x,y){
  alarmButtons.push({id:'ab_'+alarmButtons.length,x,y,r:18,cooldown:0,flash:0});
}
...
function createEnemyAt(x,y,zone=null,kind=null){
  ...
  kind = kind || (roll < .18 ? 'mirror' : roll < .46 ? 'rifle' : roll < .73 ? 'scout' : 'heavy');
  const spriteIndex = kind === 'mirror' ? 3 : kind === 'scout' ? 1 : kind === 'heavy' ? 2 : 0;
  const modes = ['patrol','guard','idle','wander'];
  const aiMode = modes[(Math.random()*modes.length)|0];
  const baseHp = kind === 'heavy' ? 90 : kind === 'scout' ? 45 : kind === 'mirror' ? 62 : 58;
...
function spawnReinforcementsNear(x,y,count=15){
  for(let i=0;i<count;i++){
    let best=null, bestZone=null, bestScore=1e9;
    for(const z of patrolZones){
      const cx = z.x + z.w/2, cy = z.y + z.h/2;
      const d = dist(x,y,cx,cy);
      if(d < bestScore){ bestScore=d; bestZone=z; }
    }
    const z = bestZone || randomZone();
    const p = randomClearPointInZone(z,70,45);
    createEnemyAt(p.x + rand(-15,15), p.y + rand(-15,15), z);
    const e = enemies[enemies.length-1];
    e.alert = 2.8;
    e.state = 'investigate';
    e.investigateX = x + rand(-120,120);
    e.investigateY = y + rand(-120,120);
  }
  message = 'POPĚCH: +'+count+' NPC';
  messageTimer = 1.8;
}
...
function updateAlarmButtons(dt){
  for(const b of alarmButtons){
    ...
    if(dist(player.x,player.y,b.x,b.y)<48){
...
function buildMap(){
  ...
  map.walls.length = 0; map.decor.length = 0; map.doors.length = 0; alarmButtons.length = 0;
  ...
  alarmButton(720,650);
  alarmButton(1390,650);
  alarmButton(2240,930);
  alarmButton(2860,1280);
  alarmButton(990,1760);
  alarmButton(2030,1760);
...
function enterLobby(){
  ...
  buildLobbyMap();
  initLobbyTargets();
  enemies = []; turrets = []; cameras = []; bullets = []; particles = []; pickups = []; shells = []; bulletMarks = []; bloodDecals = [];
...
function initLobbyTargets(){
  ...
  lobbyTargets.push({name:'FIGURÍNA',x:lobbyData.targetA.x,y:lobbyData.targetA.y,ang:lobbyData.targetA.ang,r:20,hp:80,maxHp:80,spriteIndex:0,state:'idle',blood:[],respawn:0,muzzleFx:0});
  lobbyTargets.push({name:'ČERNÝ TERČ',x:lobbyData.targetB.x,y:lobbyData.targetB.y,ang:lobbyData.targetB.ang,r:20,hp:105,maxHp:105,spriteIndex:2,state:'idle',blood:[],respawn:0,muzzleFx:0});
}
...
function hitLobbyTarget(t,b){
  ...
  t.hp -= b.dmg;
  for(let i=0;i<5;i++) t.blood.push({x:rand(-18,18),y:rand(-18,18),r:rand(4,10),a:rand(-Math.PI,Math.PI),s:rand(.8,1.6)});
  if(t.blood.length>28) t.blood.splice(0,t.blood.length-28);
  splatter(b.x,b.y,14,1.1);
  addBloodDecal(t.x+rand(-12,12),t.y+rand(-12,12),rand(10,20),false,a);
  ...
  if(t.hp<=0){
    ...
    addBloodDecal(t.x+rand(-18,18),t.y+rand(-18,18),rand(22,42),false,a);
  }
}
...
function drawLoadoutCard(card, selected){
  ...
  if(d.ready){
    ...
    ctx.fillStyle='#444'; ctx.fillRect(card.x+18, card.y+14, card.w-36, 96);
  }
  ...
  ctx.fillText(selected ? 'VYBRÁNO' : 'KLIKNOUT PRO VÝBĚR', card.x+12, card.y+206);
  ctx.restore();
}
...
function drawLobbyWindows(){
  ...
  if(lobbyUi==='shop'){
    ...
    cards.forEach(card => drawLoadoutCard(card, player.weaponType === card.key));
  }
...
function applyLoadout(key, save=true, refill=false){
  const useKey = weaponDefs[key] ? key : 'm4';
  const def = weaponDefs[useKey];
  ...
  if(refill){
    ...
    player.ammo = Math.min(player.ammo || def.clip, def.clip);
    player.reserve = Math.min(player.reserve || def.reserve, def.reserve);
  }
  updatePlayerSprite();
  if(save) saveProgress();
}
...
function resetGame(){
  ...
  applyLoadout(player.weaponType, false, true);
  player.shootCd=0; player.bt=100; player.score=0; player.kills=0; player.dead=false; player.muzzleFx=0; player.walk=0; player.moving=false; player.reloadMax=0; player.bleedTime=0; player.bleedNext=0; player.blood=[];
  enemies = []; turrets = []; cameras = []; bullets = []; particles = []; pickups = []; shells = []; bulletMarks = []; bloodDecals = [];
  securityAlarm = {x:0,y:0,timer:0,source:''};
...
function splatter(x,y,n=8,power=1){
  ...
  for(let i=0;i<drops;i++){
    addBloodDecal(x+rand(-10,10), y+rand(-10,10), rand(5,12)*power, false, rand(-Math.PI,Math.PI));
  }
}
function addBloodDecal(x,y,size=10,onWall=false,a=0){
  bloodDecals.push({
    x,y,size:size*rand(.85,1.28), onWall,
    a:a+rand(-.9,.9),
...
function sprayBloodToWall(x,y,a,maxLen=90){
  const step = 6;
  for(let d=12; d<=maxLen; d+=step){
    const px = x + Math.cos(a)*d;
    const py = y + Math.sin(a)*d;
    if(pointInWall(px,py)){
      addBloodDecal(px - Math.cos(a)*5, py - Math.sin(a)*5, rand(7,13), true, a);
      return;
    }
  }
}
...
function dropShell(x,y,a,power=1){
  ...
  if(shells.length > 90) shells.splice(0, shells.length - 90);
}
function addBulletMark(x,y,a){
  bulletMarks.push({x,y,a,size:rand(3.5,6.5)});
  if(bulletMarks.length > 700) bulletMarks.shift();
}
...
function shoot(owner, x,y,a, dmg, speed, spread=.035){
  ...
  const bx = x + Math.cos(a)*muzzleDist;
  const by = y + Math.sin(a)*muzzleDist;
...
function updatePlayerWeapon(dt, simDt){
  ...
  if(player.weaponType && mouse.down && player.shootCd<=0 && player.reload<=0 && player.ammo>0 && !player.dead){
    ...
    if(player.weaponType === 'sniper'){
      moveCircle(player, -Math.cos(player.ang)*255, -Math.sin(player.ang)*255, 1);
...
function updateLooseFx(dt, simDt){
  for(const b of bullets){
    ...
    if(pointInWall(b.x,b.y)){
      b.life = 0;
      addBulletMark(b.x,b.y,Math.atan2(b.vy,b.vx));
      for(let i=0;i<5;i++) pushParticle(b.x,b.y,rand(-70,70),rand(-70,70),rand(.15,.38),rand(1,3),'dust');
    }
  }
...
function update(dt){
    ...
    if(sees){
      ...
      for(const e of enemies){
        if(e.hp>0){
          ...
          if(!e.buttonTried && Math.random()<.60){
    ...
    if(b.owner==='p'){
      ...
      for(const e of enemies){
        if(e.hp>0 && dist(b.x,b.y,e.x,e.y)<e.r+4){
          ...
          e.alert=2.5; b.life=0; splatter(b.x,b.y,16,1.45); playSound(soundPaths.hit,.28);
          ...
          for(let bi=0; bi<6; bi++){
            ...
            e.blood.push({x:relX,y:relY,r:rand(4,10),a:rand(-Math.PI,Math.PI),s:rand(.8,1.7)});
          }
          if(e.blood.length > 54) e.blood.splice(0, e.blood.length - 54);

          // hodně krve na zem i případně na stěnu
          addBloodDecal(e.x + rand(-10,10), e.y + rand(-10,10), rand(12,22), false, hitAng);
          addBloodDecal(b.x + rand(-14,14), b.y + rand(-14,14), rand(10,18), false, hitAng);
          addBloodDecal(e.x + rand(-20,20), e.y + rand(-20,20), rand(9,18), false, hitAng);
          if(Math.random() < .96) addBloodDecal(e.x + rand(-22,22), e.y + rand(-22,22), rand(10,20), false, hitAng);
          if(Math.random() < .72) addBloodDecal(e.x + rand(-24,24), e.y + rand(-24,24), rand(7,14), false, hitAng + rand(-.4,.4));
          sprayBloodToWall(b.x,b.y,hitAng,115);
          sprayBloodToWall(e.x,e.y,hitAng + rand(-.22,.22),130);
          sprayBloodToWall(e.x,e.y,hitAng + rand(-.65,.65),165);

          if(e.hp<=0){
            player.kills++; player.totalKills++; saveProgress(); grantXp(1); player.score += e.type==='armored'?180:100; splatter(e.x,e.y,70,2.35);
            ...
            addBloodDecal(e.x + rand(-18,18), e.y + rand(-18,18), rand(38,62), false, hitAng);
            addBloodDecal(e.x + rand(-30,30), e.y + rand(-30,30), rand(24,42), false, hitAng);
            addBloodDecal(e.x + rand(-24,24), e.y + rand(-24,24), rand(18,34), false, hitAng + rand(-.6,.6));
            addBloodDecal(e.x + rand(-36,36), e.y + rand(-36,36), rand(22,48), false, rand(-Math.PI,Math.PI));
            sprayBloodToWall(e.x,e.y,hitAng,150);
            if(Math.random() < .86) sprayBloodToWall(e.x,e.y,hitAng + rand(-.7,.7),165);
      ...
      player.hp -= b.dmg; b.life=0; splatter(b.x,b.y,3); shake = player.hp - b.dmg > 0 ? 6 : 0; playSound(soundPaths.hit,.45);
      const hitAng = Math.atan2(b.vy,b.vx);
      if(player.hp > 0){
  ...
  if(!player.pickupText){
    for(const p of pickups){
      ...
      if(d<42){
        ...
        if(keys.e || d<25){
          p.dead=true;
          if(p.type==='ammo'){ player.reserve += 18; player.score += 25; }
  ...
  if(shake>0) shake = Math.max(0,shake - 32*dt);
  if(messageTimer>0) messageTimer -= dt;
}
...
function draw(){
  ...
  if(state==='loading') drawLoading();
    ...
    drawCameras();
    drawTurrets();
    for(const e of enemies){ if(e.hp<=0 || playerSeesEnemy(e)) drawEnemy(e); }
    drawBullets();
    drawParticles();
    ctx.translate(camera.x - sx, camera.y - sy);
    drawLighting();
    ...
    drawPlayerLabels();
    drawHUD();
    if(gameMode==='lobby'){ drawLobbyStatsBoard(); drawLobbyPrompts(); drawLobbyWindows(); }
    drawMessage();
...
function drawAlarmButtons(){
  ...
  for(const b of alarmButtons){
    ...
    ctx.beginPath(); ctx.ellipse(5,7,24,14,0,0,Math.PI*2); ctx.fill();
    ctx.fillStyle=blink?'#ff2b2b':'#8b000b';
    ...
    if(b.cooldown>0){
      ...
      ctx.lineWidth=3;
      ctx.beginPath(); ctx.arc(0,0,b.r+5,-Math.PI/2,-Math.PI/2+Math.PI*2*(1-b.cooldown/18)); ctx.stroke();
    }
    ctx.restore();
  }
}
...
function drawBulletMarks(){
  for(const bd of bloodDecals){
    ...
    const baseDark = bd.onWall ? 'rgba(110,6,6,'+bd.alpha+')' : 'rgba(96,0,0,'+bd.alpha+')';
    const baseMid = bd.onWall ? 'rgba(160,18,18,'+(bd.alpha*.55)+')' : 'rgba(155,10,10,'+(bd.alpha*.50)+')';
    const baseHi = bd.onWall ? 'rgba(215,45,45,'+(bd.alpha*.20)+')' : 'rgba(205,35,35,'+(bd.alpha*.16)+')';
...
function drawPlayer(){
  ...
  if(playerSpriteReady){
    ctx.imageSmoothingEnabled = true;
    try { ctx.imageSmoothingQuality = 'high'; } catch(e) {}
    ctx.filter='brightness(0.40) contrast(1.18) saturate(.78)';
...
function drawEnemy(e){
  ...
  if(ready && img){
    ...
    for(const bm of (e.blood || [])){
      ...
      ctx.rotate(bm.a);
      ctx.scale(bm.s, 1);
      ctx.fillStyle = e.hp > 0 ? 'rgba(165,8,8,.92)' : 'rgba(75,4,4,.86)';
      ctx.beginPath();
      ...
      ctx.fill();
      ctx.strokeStyle = e.hp > 0 ? 'rgba(130,0,0,.52)' : 'rgba(55,0,0,.44)';
      ...
      ctx.stroke();
      ctx.fillStyle = e.hp > 0 ? 'rgba(235,46,46,.22)' : 'rgba(115,18,18,.16)';
...
function drawHUD(){
  ...
  const ammoRes = '/ ' + String(player.reserve);
...
function drawMinimap(){
  const mw=260,mh=175,x=W-mw-24,y=H-mh-24;
  ctx.fillStyle='rgba(0,0,0,.86)'; ctx.strokeStyle='#d20b1c'; ctx.lineWidth=2; ctx.fillRect(x,y,mw,mh); ctx.strokeRect(x,y,mw,mh);
  const sx=mw/map.w, sy=mh/map.h;

  // Fog-of-war: mapa se odkrývá jen tam, kde hráč prošel.
  if(fog.seen.length){
    for(let cy=0; cy<fog.rows; cy++){
      for(let cx=0; cx<fog.cols; cx++){
        ...
        const wx = cx*fog.cell + fog.cell/2;
        const wy = cy*fog.cell + fog.cell/2;
        ctx.fillStyle = pointInWall(wx,wy) ? '#5a5a5a' : '#242424';
  ...
  ctx.fillStyle='#d20b1c'; ctx.beginPath(); ctx.arc(x+player.x*sx,y+player.y*sy,3.5,0,Math.PI*2); ctx.fill();
  // Enemy body / dots are intentionally hidden. You must spot them with the flashlight.
  ctx.font='10px Arial'; ctx.fillStyle='rgba(255,255,255,.55)'; ctx.fillText('MAPA',x+8,y+14);
}
      ...
      if(lobbyUi==='loadout'){
        for(const card of getLoadoutCardRects()){
          if(inb(card.x, card.y, card.w, card.h)){
            applyLoadout(card.key, true, true);
